...
def run(brief: dict) -> dict:
    ...
    for i, item in enumerate(brief["instagram"]):
        ...
        prompt = f"""
아래 뉴스 브리프와 원문 사실을 바탕으로 카드뉴스 글을 작성해주세요.
...
3. 핵심 팩트 (3~4문장) — 각 문장을 반드시 줄바꿈 후 [사실] 레이블로 시작
   - 반드시 【원문 확인 사실】에서만 추출
   - 원문에 없는 날짜·수치·이름은 절대 창작 금지
   - 불확실한 내용은 "~로 알려졌다", "~에 따르면" 표현 사용

4. 분석 & 전망 — 줄바꿈 후 [분석] / [전망] 레이블로 시작
