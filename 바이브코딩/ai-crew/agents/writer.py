...
첫 줄: 이모지 하나 + 스크롤 멈추게 하는 한 문장 (질문 또는 반전).
그 다음: 왜 중요한지 배경 1~2문장, 핵심 사실 2~3줄.
마지막: 짧은 생각이나 여운 한 문장 (마침표로 끝내기).

지킬 것:
- 질문은 글 전체에서 딱 1개만. 두 번째 질문 절대 금지.
- 같은 표현·단어 반복 금지 ("이러한", "이런 상황", "어떻게 될지" 등)
- 페르소나 직접 언급 금지 — 느낌으로만
- 없는 수치·사실 창작 절대 금지
- 한글 단어를 한자로 표기하거나 혼용 금지 (車량, 金錢적인 같은 표현 절대 금지)
- 한국어와 영어만. 독일어·베트남어 등 외국어 단어 금지
- 레이블([사실], [분석] 등) 달지 마
- 해시태그는 반드시 실제 단어로만. 빈 해시태그(# 뒤에 아무것도 없는 것) 절대 금지
- 전체 350~450자

글 다 쓰고 빈 줄 두 개 띄우고 해시태그 8개 써줘.
"""
...
글의 흐름: 왜 생겼는지(배경) → 무슨 일인지(내용) → 어떤 의미인지(해석)

뉴스 봤을 때 첫 반응으로 시작해 (인사말 금지).
뉴스 모르는 사람도 배경부터 이해하게 설명해.
소제목(##) 2개만, 대화체로. 보고서 제목 금지.
내 생각 자연스럽게 — 근데 "이러한", "이처럼", "이를 통해" 같은 표현 반복 금지.
문단마다 새로운 내용. 같은 말 다른 표현으로 반복하는 것도 금지.
마지막은 짧고 여운 있게 마침표로. "내일도 좋은 뉴스" 같은 공허한 마무리 금지.
분량 700~900자. 할 말 없으면 억지로 늘리지 마 — 짧아도 됨.
한글 단어를 한자로 쓰거나 혼용 금지. 한국어만.
"""
