...
- 각 뉴스의 각도(angle), 톤, 키워드, **원문 확인 사실(source_facts)** 기획
  - 카테고리당 최대 3개 기사 본문(400자) 직접 전달 → 구체적 사실 추출
  - HTML 엔티티 자동 디코딩 (`&amp;` → `&` 등)
...
**페르소나**: 40대 BMW 딜러 직원 '시아아빠' — 매일 아침 뉴스 읽고 블로그에 느낀 점 기록
**담당**:
- 카드뉴스 글 **5개** (400~600자)
  - 첫 줄: 이모지 1개 + 사람 멈추게 하는 한 문장
  - 섹션 레이블 없이 자연스러운 대화체로 작성
  - **source_facts 기반 작성** — 원문에 없는 수치·이름·날짜 창작 금지
  - 해시태그 8개로 마무리
- 블로그 아티클 1개 (800~1200자, 소제목 2~3개)
  - 뉴스 봤을 때 첫 반응/감정으로 시작 (형식적 인사말 금지)
  - 중간에 개인 의견/해석 자연스럽게 포함
  - 블로그도 **source_facts 기반** — 불확실한 내용은 "~로 알려졌다" 표현 사용
- 카드 간 5초 대기 (Groq TPM 한도 대응)
...
### 2026-04-03
- **작가 페르소나 변경**: "30년 경력 수석 저널리스트" → **시아아빠** (40대 BMW 딜러 직원)
  - 카드뉴스: 섹션 레이블([사실]/[분석]/[전망]) 제거, 자연스러운 대화체, 400~600자, 해시태그 8개
  - 블로그: 분량 1500자+ → **800~1200자**, 소제목 4개+ → **2~3개**, temperature 0.7→0.88
- **기획자 source_facts 버그 수정**: 원문 기사 내용이 항상 비어있던 문제 해결
  - `a.get("body","")` → `a.get("summary","") or a.get("body","")` (실제 데이터 키 수정)
  - `html.unescape()` 적용 — `&amp;`, `&quot;` 등 HTML 엔티티 정상 처리
  - 기사 본문 최대 400자를 플래너에 직접 전달 (기존: 제목만 전달하여 source_facts 공백)
  - 카테고리당 기사 수: 2→3개
...
*최종 업데이트: 2026-04-03 | Powered by Groq + Pollinations.ai + Notion API + GitHub Pages*
