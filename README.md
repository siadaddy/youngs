...
└─ docs/trades.json (블랙리스트 포함) → GitHub Pages push
...
| IP 변경 감지 | 공인 IP 변경 시 ntfy 긴급 알림 |
...
- **기존 손절 기록 자동 등록**: NCT 3회(7일 차단), FORT 2회(3일 차단) 즉시 적용
- **투자금 축소**: 10만원 → 5만원 / 일일손실한도 -15,000원 → -7,500원
- **buy_market_order 버그 수정**: amount_krw 파라미터 무시 버그 (avail_krw×0.75 재계산) 수정
- **기회교체 쿨다운**: 기회교체 SELL 후 해당 종목 2시간 재진입 금지 추가
- **즉시 재판단 시 데이터 재수집**: 동일 데이터로 재판단하던 문제 수정
- **스테이블코인 하드차단**: executor에서 USDT/USDC 등 코드 레벨 BUY 차단
- **종목 필터 강화**: 24h 거래대금 20억원 미만 잡코인 분석 제외
- **RSI 스코어링 수정**: RSI<20은 급락 추세로 판단, 중립(0점) 처리
- **VB 조건 강화**: 거래량 +20% 동반 시 +2점 / 미동반 시 +1점
- **ADX 기준 강화**: 15→20 이상으로 상향
...
- **익절 수정**: +8% → +5% (수익 자주 챙기기)
- **AI 시간 기반 매도 규칙**: 보유 시간별 매도 기준 완화 (4h·6h 분기점)

### 2026-04-14
- **코인 트레이더 안정성**: 빗썸 API 불안정 시 `get_krw_tickers()` None 크래시 수정
- **Groq 429 즉시 전환**: 90초 대기 → 즉시 key 전환
- **작가 품질 게이트 전면 개편**: 5단계 검증 + targeted hint 재생성
...
*최종 업데이트: 2026-04-21 | Powered by Groq + Pollinations.ai + pybithumb + Three.js + GitHub Pages*
