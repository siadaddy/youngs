...
| **AI 사무실** | **픽셀아트 Canvas 사무실 — 8명 AI 직원 실시간 행동 + 학습 기록 말풍선** |
| 소개 | 프로젝트 소개 & AI 크루 |
...
Canvas 픽셀아트 사무실에서 AI 직원 8명(박기획·최디자·한뮤직·AI주간트렌드·AI어드바이저·뉴스기자·이가드·리포터)이 돌아다니며 **실제 학습 데이터**를 말풍선으로 표시합니다.

- **스프라이트**: 16×24 고화질 픽셀아트 (캐릭터별 고유 디자인 — 헤드폰·중절모·발광 눈 등)
- **행동 19가지**: 산책·커피·창문·스트레칭·고민·전화·춤·독서·화이트보드·간식·동료 대화·프린터·졸기·순찰·보고·물 마시기·운동 등
- **회의**: 2~3분마다 전원 집합 → 실제 작업 내용 브리핑
- **학습 데이터 연동**: `office_memory.json` ← 매매 이력·이미지 성공률·반복 키워드 등 실데이터
...
| 코인 API | pybithumb (빗썸) |
| 음악 데이터 | YouTube Data API v3 · video_cache.json |
| 유튜브 플레이리스트 | "AI 추천 플레이리스트" 고정, 신곡만 누적 추가 |
| 자동화 | macOS launchd |
| 알림 | ntfy |
| 호스팅 | GitHub Pages |
| 비용 | **$0 (전부 무료)** |
