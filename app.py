...
with 탭_전략:
    ...
    strategies = [
        ("다양한 상품 경험 유도", 'unique_product_count', '고유 상품 수 확대'),
        ("재방문 유도", 'reorder_rate', '재구매율 상승'),
        ("휴면 방지", 'recency', '방문 주기 단축'),
        ("장바구니 크기 증가", 'avg_cart_size', '장바구니 크기 확대')
    ]
    for title, metric, label in strategies:
        with st.expander(f"📌 {title}"):
            st.markdown(f"- 1등급 고객의 {label}가 [그룹별 평균 비교]를 통해 확인됩니다.")
