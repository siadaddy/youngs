...
{{
  "instagram": [
    {{
      ...
      "reader_insight": "독자가 이 카드를 읽고 얻어가야 할 핵심 관점 1문장. 시아아빠 목소리로. 예: '혼다 철수 빈자리를 중국 전기차가 채운다는 게 진짜 문제야' / '금리 동결이 지금 내 대출에 바로 이런 영향을 줘'",
...
- 🏢 삼천리 그룹 카테고리에 사업전략·실적·신사업·그룹 동향 관련 뉴스가 있으면 5개 카드 중 하나에 반드시 포함할 것. 단, 골프/스포츠 선수 협찬·단순 지역 행사·복지 안내는 제외. 삼천리 뉴스가 없거나 해당 기준 미달이면 일반 규칙으로 선별
